...
namespace algebra
{
	...
	public: // operators
        ...
        friend Vertex<int> operator+(const Vertex<int> &A, const Vertex<int> &B);
        friend Vertex<int> operator+(const Vertex<int> &A, const Vertex<double> &B);
        friend Vertex<int> operator+(const Vertex<int> &A, const Vertex<float> &B);
	};
    std::ostream& operator<<(std::ostream& os, Vertex<int> vertex);
    Vertex<int> operator+(const Vertex<int> &A, const Vertex<int> &B);
    Vertex<int> operator+(const Vertex<int> &A, const Vertex<double> &B);
    Vertex<int> operator+(const Vertex<int> &A, const Vertex<float> &B);
}